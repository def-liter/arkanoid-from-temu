...
rows = randint(2, 6)
cols = randint(3, 8)
current_block_chanel = 1
pygame.mixer.set_num_channels(rows * cols + 1)
print(rows, cols)
padding = 10
offset_x = 20
offset_y = 40
...
def cube_colision():
    global c_x, c_y, c_x_speed, c_y_speed, current_block_chanel

    if cube.colliderect(platform):
    ...
    for brick in bricks[:]:
        if cube.colliderect(brick):
            print(f"block chanel:{current_block_chanel}")
            pygame.mixer.Channel(current_block_chanel).play(pygame.mixer.Sound("sound/block_dink.mp3"))
            current_block_chanel += 1
    ...
    if c_y + 50 >= SCREEN_HEIGHT:
        you_lose()
    return c_x_speed, c_y_speed, current_block_chanel
...
while True:
    ...
    cube = pygame.draw.rect(screen, (0, 0, 255), (c_x, c_y, 50, 50))
    platform = pygame.draw.rect(screen, (0, 255, 0), (x, y, 175, 25))
    #draws the bricks in the bricks list
    for brick in bricks:
        pygame.draw.rect(screen, (random_col_at_start), brick)
